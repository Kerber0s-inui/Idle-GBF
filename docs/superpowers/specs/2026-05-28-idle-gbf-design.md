# Idle GBF 设计规格

日期：2026-05-28

## 概要

本项目是一个手机优先的本地 PWA 放置 RPG。外层循环参考 Whipper：选择副本、选择周回次数、等待周回完成、领取奖励、强化队伍、继续推进。内层养成参考《碧蓝幻想》式的队伍、武器盘、召唤石和伤害公式分区。

第一版优先做出完整可玩的系统闭环，而不是铺开大量内容。首版只做一条路线：火属性队伍对风属性敌人。数据结构需要为后续六属性扩展预留空间。

## 核心循环

1. 玩家推进主线副本。
2. 未首通副本需要进行一次轻量头像/像素自动战斗。
3. 副本通关后记录首通状态，并解锁该副本的扫荡周回。
4. 扫荡周回时，玩家选择周回次数，上限为 100 次。
5. 每次周回耗时由副本难度决定，大约 5 到 10 分钟。
6. 完成的扫荡周回会汇总结算经验、素材、武器、召唤石和有限抽卡资源。
7. 玩家强化角色、武器、召唤石、被动技能和武器盘。
8. 强化后的队伍继续挑战下一个未首通副本。

常规副本没有回合限制。挑战战斗会持续到敌人被击败或我方队伍被击败。失败只需要简洁结果，例如“挑战失败”或“队伍被击败”。高难副本特殊机制明确不进入第一版范围。

## 平台

第一版是本地 PWA：

- 手机优先布局。
- 本地存档。
- JSON 存档导入/导出。
- 不做后端账号系统。
- 不做付费货币或商业化。
- 不做服务端防作弊。

这样可以优先完成可玩的本地版本，同时保留以后迁移到桌面端或带服务器版本的空间。

## 视觉与素材策略

使用两套素材模式：

- 开发/本地模式可以把 asset key 映射到 GBFAL ID 或 URL。
- 发布模式把同一套 asset key 映射到占位素材或原创素材。

项目不能提交、再分发或打包 Cygames/Granblue Fantasy 素材。本地缓存目录必须加入 `.gitignore`。玩家可见名称必须原创，不能复制 GBF 原名。

命名风格为奇幻 + 魔法科技。可以使用炉心、回路、星轨、协议、机神、浮空岛、星能引擎等意象。名称应该像 RPG 角色或装备，而不是“火属性 SSR 攻刃剑”这种直白功能标签。

## 内容范围

第一版内容：

- 一条火属性队伍对风属性敌人的主路线。
- 通过主线进度解锁免费角色。
- 使用游戏内资源的抽卡系统，可产出角色、角色关联武器、其他武器和召唤石。
- 副本掉落与武器盘相关的武器、召唤石、素材、经验和有限抽卡资源碎片。
- 一条可通过免费周回成型的武器盘路线，精神上接近 GBF 的可农武器成长。

第一版不做：

- 完整六属性内容。
- 高难副本特殊机制。
- PVP。
- 公会。
- 活动与活动排期。
- 付费系统。
- 联网账号。
- 完整 debuff/buff 生态。
- 角色主动技能。
- 复杂战斗演出。
- GBF 素材再分发。

## 角色设计

角色通过基础数值、属性、稀有度、两个被动技能和奥义来区分。

第一版战斗动作只有：

- 普通攻击。
- 奥义。

第一版不包含角色主动技能。

每个角色固定有两个被动槽。被动可以影响：

- 个人攻击或防御。
- 全队攻击分区。
- 奥义槽获取。
- 奥义伤害或奥义上限。
- 连击率。
- 暴击率或暴击伤害。
- 伤害上限。
- 生存能力。
- 扫荡效率。
- 掉落加成。

角色成长包括等级、突破/进阶、被动解锁或强化、奥义强化。

## 战斗设计

首通战斗使用轻量头像或像素表现。战斗仍然完全自动。

战斗流程：

1. 初始化队伍、敌人、武器盘、召唤石、被动和公式修正。
2. 每回合按队伍顺序行动。
3. 角色奥义槽满足条件时释放奥义。
4. 否则角色进行普通攻击。
5. 敌人行动自动结算。
6. 战斗在敌人被击败或我方队伍被击败时结束。

战斗结果需要包含足够结构化的数据，供 UI 和测试使用：

- 胜利或失败。
- 回合日志。
- 伤害记录。
- 奥义事件。
- 被动生效或触发记录。
- 最终我方和敌方状态。

扫荡模式不播放每次战斗。它基于首通状态和副本配置生成按时间完成的任务，并在结算时汇总奖励。

## 公式设计

公式模块必须独立于 UI、存储和战斗流程。它应该尽量按 GBF 风格的分区搭建：

- 通常攻刃修正。
- 方阵/可农攻刃修正。
- EX 攻刃修正。
- 召唤石加护修正。
- 属性克制。
- 暴击率和暴击期望。
- 连击。
- 奥义倍率。
- 伤害上限。
- 伤害上限提升。
- 被动修正。

实现备注需要说明哪些公式部分是近似实现或刻意简化。这样后续校准时不会忘记哪些地方是暂定值。

编成界面需要解释主要贡献分区，让玩家理解为什么某次换盘会提升或降低表现。

## 词条与上限设计

第一版词条体系以“能形成 GBF 风格武器盘构筑”为目标，但控制范围，避免一开始就引入追击、补伤、弱体和高难机制。词条分为首版必做、首版少量做、首版先不做三类。

### 首版必做词条

| 词条 | 效果 | 上限或规则 |
| --- | --- | --- |
| 通常攻刃 | 提升通常攻击分区 | 同分区相加，无独立硬上限，主要受武器格数量限制 |
| 方阵攻刃 | 提升方阵/可农攻击分区 | 同分区相加，可被方阵召唤加护放大 |
| EX 攻刃 | 提升 EX 攻击分区 | 同分区相加，不吃方阵/主神类加护 |
| 属性攻击 | 提升元素分区 | 火克风基础克制倍率为核心，召唤石或被动可继续增加 |
| 通常守护 | 提升 HP | 同分区相加，按通常类加护规则处理 |
| 方阵守护 | 提升 HP | 同分区相加，可被方阵召唤加护放大 |
| 浑身 | HP 越高攻击越高 | 按当前 HP 比例计算，不单独设置硬上限 |
| 背水 | HP 越低攻击越高 | 按当前 HP 比例计算，不单独设置硬上限 |
| 暴击率 | 普攻和奥义有概率暴击 | 最终暴击率最高 100% |
| 暴击伤害 | 暴击时提升伤害 | 首版默认暴击伤害为 150%，特殊词条可增加 |
| 连击率 | 决定 DA/TA | DA 和 TA 各自最高 100%，判定时 TA 优先 |
| 奥义伤害 | 提升奥义倍率 | 装备来源建议软上限为 +100% |
| 奥义上限 | 提升奥义伤害上限 | 首版建议总上限为 +50% |
| 伤害上限 | 提升所有伤害软上限 | 装备来源建议上限为 +20%，角色被动可作为独立来源另算 |
| 普攻上限 | 只提升普通攻击上限 | 首版建议总上限为 +50% |
| 防御/减伤 | 降低受到伤害 | 最终减伤建议封顶 70% |
| 掉落率 | 提升素材和装备掉落期望 | 首版建议总上限为 +50%，不影响首通奖励 |
| 扫荡效率 | 缩短扫荡耗时 | 首版建议总上限为 -30%，且不能低于副本配置的最短耗时 |

### 首版少量做的词条

| 词条 | 效果 | 上限或规则 |
| --- | --- | --- |
| 克制增幅 | 只在属性克制时增伤，承担类似天司武器的构筑位置 | 首版建议上限为 +20%，作为稀有词条出现 |
| 独立攻刃 | 由角色被动或特殊召唤提供的独立乘区 | 数值少给，不吃通常/方阵/EX 加护 |
| 奥义回转 | 提升奥义槽获取量 | 首版建议上限为 +100%，避免过早形成永久奥义循环 |
| 开局奥义槽 | 战斗开始时获得奥义槽 | 最高 100% |
| 召唤加护强化 | 提升指定类型武器技能效果 | 只作用于指定分区，避免全局乱乘 |

### 首版先不做的词条

第一版先不做技能伤害、技能上限、追击/echo、补充伤害、反击、连锁爆发、弱体成功率、弱体耐性、特殊上限、hit 数相关词条，以及“第 N 回合”“敌方状态”“多武器标签联动”等复杂条件词条。

### 连击与奥义获取

连击不只是提升普通攻击伤害，也会直接影响奥义槽获取。第一版暂定规则：

- 单次攻击获得基础奥义槽。
- DA 视为两次攻击，获得约 2 倍基础奥义槽。
- TA 视为三次攻击，获得约 3 倍基础奥义槽。
- 奥义回转词条在连击结算后乘算或加算到最终奥义槽获取，具体公式在实现计划阶段固定。
- 如果角色被动提供“连击时额外奥义槽”，该效果作为被动修正单独记录，方便 UI 展示来源。

这样连击率会同时影响普通攻击期望和奥义频率，让角色被动、武器盘和奥义体系产生联动。

## 远征与扫荡规则

副本字段应包括：

- 副本 ID。
- 属性。
- 难度层级。
- 单次周回耗时。
- 首通敌人配置。
- 掉落表。
- 首通奖励。
- 解锁条件。

扫荡规则：

- 只有首通后可用。
- 玩家选择周回次数。
- 周回次数上限为 100。
- 总耗时等于单次耗时乘以周回次数。
- 完成前玩家可以查看进度。
- 结算时汇总所有已完成周回。

第一版可以用时间作为主要节奏限制。AP、门票、体力或其他入场资源可以以后再加。

## 奖励与经济

奖励分为两条线：

- 推进奖励：主线和首通奖励提供免费角色、大量抽卡资源、关键突破素材和里程碑解锁。
- 周回奖励：扫荡提供武器、召唤石、技能素材、角色成长资源、经验和有限抽卡资源碎片。

抽卡资源不能通过扫荡高速无限产出。周回可以缓慢贡献，但大额抽卡资源主要来自主线推进。

武器和召唤石获取使用类似 GBF 的混合模式：

- 抽卡可以获得角色、武器和召唤石。
- 副本可以掉落可农武器和召唤石。
- 免费农出来的武器盘应该足以完成首版路线。

## UI 结构

使用手机优先的 PWA 界面，底部有五个导航入口：

- 远征：主界面，用于副本选择、首通挑战、扫荡设置、进度查看和奖励结算。
- 编成：队伍、武器盘、召唤石、战力摘要和公式拆解。
- 强化：角色、武器、召唤石、被动、突破和技能强化。
- 抽卡：资源抽卡、卡池详情和抽卡记录。
- 仓库：素材、武器、召唤石、掉落历史、存档导入/导出和素材模式设置。

首页体验应优先服务远征。编成和强化要足够近，但玩家打开游戏时首先应该看到当前正在刷什么、下一步要挑战什么。

## 数据模型

核心实体：

- Character。
- Passive。
- ChargeAttack。
- Weapon。
- WeaponSkill。
- Summon。
- Quest。
- Enemy。
- PartyLoadout。
- WeaponGrid。
- Inventory。
- PlayerProgress。
- GachaPool。
- RewardTable。
- BattleResult。
- ExpeditionRun。
- SaveFile。

存档必须包含版本号。导入存档时应做 schema 校验，并在可能时迁移旧版本数据。

## 架构

建议实现架构：

- TypeScript PWA 前端。
- React/Vite 或类似的轻量前端栈。
- 公式、战斗、远征结算、奖励、抽卡和存档迁移使用纯 TypeScript 领域模块。
- UI 组件调用领域模块，但不拥有核心规则。
- 资源解析器把稳定的 asset key 映射到开发/本地 GBFAL 引用，或发布模式的占位/原创文件。

建议模块边界：

- `formula`：伤害和修正计算。
- `battle`：首通自动战斗模拟。
- `expedition`：扫荡耗时和结算。
- `progression`：解锁、主线进度、首通标记。
- `inventory`：已拥有角色、武器、召唤石、素材和货币。
- `gacha`：卡池、概率和抽卡结果。
- `save`：持久化、导入/导出、schema 校验和迁移。
- `assets`：asset key 解析和素材模式切换。
- `ui`：PWA 页面和展示层。

## 测试策略

优先测试领域逻辑，再做 UI 打磨：

- 公式测试覆盖攻刃分区、召唤加护、属性克制、暴击期望、奥义、上限和被动修正。
- 战斗测试覆盖胜负、奥义时机、普通攻击、敌人伤害和被动应用。
- 远征测试覆盖首通门槛、周回次数上限、耗时计算、进度和结算。
- 奖励测试覆盖掉落汇总、首通奖励和有限抽卡资源生成。
- 存档测试覆盖导入、导出、校验和版本迁移。

UI 测试至少覆盖手机端主流程：

1. 开始首通战斗。
2. 胜利并解锁扫荡。
3. 选择次数开始扫荡。
4. 结算奖励。
5. 强化并调整编成。
6. 使用获得的资源抽卡。
7. 导出和导入存档。

## 实现计划阶段待定项

- 具体前端框架和 UI 库。
- 初始角色、武器、召唤石和素材命名。
- 精确 GBF 公式参考来源和第一版近似清单。
- 首条路线的副本数量和推进节奏。
- 扫荡完成是否第一版就使用真实墙钟时间，或在开发模式提供测试用加速时钟。
